...
def htmlize(adict):
    new = adict.copy()
    for field in fieldnames:                        # values may be &, >, etc.
        value = new[field]                          # display as code: quote
...
def update_record(db, form):
    if not 'key' in form:
        ...
        if key in db:
            ...
            record = Person(name='?', age='?')      # eval: string must be quoted
...
if action == 'Fetch':
    ...
    fields.dictfromkeys(fieldnames, '?')            # bad submit button value
