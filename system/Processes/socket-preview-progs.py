...
sockets are machine-global, much like fifos: dont require shared memory
"""
...
if mode == 1:                                   # run server in this process
    ...
    client(f'client:process={os.getpid()}')
else:                                           # run 5 client threads in process
    for i in range(5):
        Thread(target=client, args=(f'client:thread={i}',)).start()